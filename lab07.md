...
prepare_set <- function(file_name) {
    ...
    # розділити текст за символом "(" з'являється список, який складається
    # з багатьох векторів. В кожному векторі перший елемент - назва країни.
    cn <- strsplit(df$Country, "(", fixed=TRUE)
    # залишити тільки назви країн - взяти перший елемент кожного вектора
    df$Country <- sapply(cn, "[", 1) # можна записати: sapply(cn, function(x) x[1])
...
answer_six <- function() {
    ...
    # 2:4 означає вибрати з 2-го до 4-го рядка (пропустити суму по штату)
